...
const hourDisplayElement = document.querySelector('.hour-display');
const minDisplayElement = document.querySelector('.min-display');
const secDisplayElement = document.querySelector('.sec-display');
...
function displayTime(timeZone) {  
  ...
  hourDisplayElement.innerHTML = formattedTime.split(':')[0] + ':';
  minDisplayElement.innerHTML = formattedTime.split(':')[1] + ':';
  secDisplayElement.innerHTML = formattedTime.split(':')[2];
...
function optionAdd(selectedRegion) {
  ...
  if (timeZoneData[selectedRegion]) {
    timeZoneData[selectedRegion].forEach((timeZone) => {
      timeZoneSelectHTML += `<option value="${timeZone}">${timeZone.replace('_', " ")}</option>`;
    });
    ...
    regionsArray.forEach(async (area) => {
      ...
      regionHTML += `
        <div class='region-box'>
          <p class='random-timeZone'>${timeZone.replace("_", " ")}</p>
...
timeZoneSelect.addEventListener('change', () => {
  ...
  currentInterval = setInterval(() => {
    regionDisplayElement.innerHTML = `Time in <span>${timeZoneValue.replace("_", " ")}, ${regionValue}</span> now: `;
